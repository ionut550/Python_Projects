...
def count_down(count):
    ...
    seconds = count%60
    canvas.itemconfig(timer_text, text=f"{minutes}:{f'0{seconds}' if seconds < 10 else seconds}")
    if count > 0:
        global timer
        timer = window.after(1000, count_down, count - 1)
    else:
        start_timer()
        check_mark_label.config(text=["✔" for _ in range(math.floor(reps/2)) ],)
# ---------------------------- UI SETUP ------------------------------- #
window = Tk()
...
timer_text = canvas.create_text(103, 132, text="00:00", fill="white", font=(FONT_NAME, 30, "bold"))
